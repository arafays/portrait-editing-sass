...
- Demo generation cap of 3 per browser, enforced **client-side only** in `components/HairStudio.tsx` via `lib/generationLimit.ts` (localStorage, same mechanism as `lib/generationHistory.ts`; the product is stateless server-side). Only successful generations count; failed ones don't. History and the counter are independent keys — clearing history does not reset the cap.
